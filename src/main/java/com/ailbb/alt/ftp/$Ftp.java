...
public class $Ftp {
    ...
    public $Ftp($ConnConfiguration connConfiguration) {
        this.setConnConfiguration(connConfiguration).login();
    }

    public $Ftp(String host, int port, String userName, String pwd) {
    ...
    public $Ftp login($ConnConfiguration connConfiguration) { // 需要重新实例化出来一个
        $Ftp ftp = new $Ftp(connConfiguration);
        return ftp.login();
    }
    ...
    public $Result logout()  {
        ...
        if (null != ftpClient && ftpClient.isConnected()) {
            try {
                ...
                rs.addMessage(logger.warn("退出服务器异常：[" + e + "]"+ connConfiguration.getIp()));
            } finally {
                try {
                    ftpClient.disconnect();    //关闭ftp服务器连接
                } catch (IOException e) {
                    rs.addMessage(logger.warn("关闭服务器异常：[" + e + "]"+ connConfiguration.getIp()));
                }
            }
        }
    ...
    public  $Result readNewFile(String remoteUpLoadPath){
        ...
        try {
            ...
            if(!login().isLogin()) return rs.addMessage("未登录！").setSuccess(false);
    ...
    public $Ftp setTmpSuffix(String tmpSuffix) {
    ...
    public $Ftp setCompress(boolean compress) {
        isCompress = compress;
        return this;
    }
    ...
    public $Ftp setCompressType(String compressType) {
    ...
    public $Ftp setTmpDir(String tmpDir) {
        this.tmpDir = tmpDir;
        return this;
    }
    ...
    public $Ftp setCompressEnableSize(long compressEnableSize) {
